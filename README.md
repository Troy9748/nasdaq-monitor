...
- 网页展示当前生效的四项告警阈值。
- 每个更新日使用 1990 年以来全部收盘价重算 Huber 稳健对数线性趋势，并在正常点位尺度展示拟合路径、历史 10%–90% 残差区间、当前偏离度和隐含长期年化增速；极端上涨和下跌样本会自动降权。
...
- 稳健长期趋势以 `log(收盘价)` 对时间做 Huber 迭代加权回归后指数还原；它是全历史统计中枢，不是估值模型或目标价。
